(function() {
    ...
    publish = function(data, opts) {
	    ...
	    print("   to: "+outdir);
	    fs.mkPath(outdir);
	    var path = outdir + '/' + 'test.xml';
	    print("   path: "+path);
	    var parts = [];
	    data.forEach(function(doclet) {
	        doclet.ancestors = generateAncestry(doclet);
		    parts.push(  require('jsdoc/util/dumper').dump(doclet)  );
		    //parts.push(  convertToXml(doclet)  );
	    });
	    var str = "{\"docs\":{\"doc\":[\n"+parts.join(",\n")+"\n]}}";
	    str = str.replace(/\<Object\>/g, "\"OBJECT\"");
	    str = str.replace(/\<anonymous>~core/g,"core");

	    var xml = convertToXml( JSON.parse(str) );
