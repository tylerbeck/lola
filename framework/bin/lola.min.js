(function(c){var d={window:c,guid:0,initialized:false,initializers:[],dependencies:{},safeDeleteHooks:[],urlvars:{},hash:"",debugMode:false,initialize:function(a){if(!d.initialized){d.debug("lola::initialize");d.initialized=true;c=a;document.addEventListener?document.removeEventListener("DOMContentLoaded",d.initialize,false):document.attachEvent&&document.detachEvent("onreadystatechange",d.initialize);d.checkDependencies(d.dependencies);for(var e=d.initializers.length,a=0;a<e;a++){var g=d.initializers[a];
typeof g=="function"&&g();delete d.initializers[a]}}},checkDependencies:function(a){var c=[],g;for(g in a){var d=this.checkModules(a[g]);d.length>0&&c.push(g+": "+d.join(", "))}if(c.length>0)throw Error("module dependency checks failed for: \n\t"+c.join("\n\t"));},checkModules:function(a){var c=[];Object.forEach(a,function(a){d.hasPackage(d,a)||c.push(a)});return c},getPackage:function(a,c){var g=a;if(typeof c==="string")for(var d=c.split("."),j;j=d.shift();)g[j]==null&&(g[j]={}),g=g[j];return g},
hasPackage:function(a,c){var g=a;if(typeof c==="string")for(var d=c.split("."),j;j=d.shift();)if(g[j]==null)return false;else g=g[j];return true},extend:function(a,c,g,d){g==void 0&&(g=false);d==void 0&&(d=false);for(var j in c)if(g||a[j]==null)a[j]=c[j];else if(d)throw Error("property "+j+" already exists on extend target!");},evaluate:function(a,c){if(c==null&&(c=document.getElementsByTagName("head")[0],!c))c=document.documentElement;var d=document.createElement("script");d.type="text/javascript";
this.support.domEval?d.appendChild(document.createTextNode(a)):d.text=a;c.insertBefore(d,c.firstChild);c.removeChild(d)},loadScript:function(a,c){console.info("loadScript: "+a);var g=document.getElementsByTagName("head")[0];if(!g)g=document.documentElement;var h=document.createElement("script");h.type="text/javascript";h.src=a;typeof c=="function"&&d.event.addListener(h,"load",function(){c.call()});g.insertBefore(h,g.firstChild)},registerModule:function(a){var c=a.getNamespace();d.debug("lola::registerModule - "+
c);a.hasOwnProperty("getDependencies")&&typeof a.getDependencies=="function"&&(d.dependencies[c]=a.getDependencies());d.extend(d.getPackage(d,c),a);d.extend(d.Selector.prototype,a.getSelectorMethods());delete a.getSelectorMethods;a.initialize&&typeof a.initialize==="function"&&d.addInitializer(function(){a.initialize()});a.preinitialize&&typeof a.preinitialize==="function"&&a.preinitialize()},safeDelete:function(a,c){for(var d=c?a[c]:a,h=this.safeDeleteHooks.length-1;h>=0;h--){var j=this.safeDeleteHooks[h];
j.fn.call(j.scope,d)}a&&c&&delete a[c]},toString:Object.prototype.toString,checkArgs:function(a,c,g){for(var h=true,j=[],k=c.length-1;k>=0;k--)if(c[k][1]===void 0||c[k][1]===null)h=false,j.push(c[k][0]+" is not set!");h||(console.groupCollapsed?console.groupCollapsed(a):console.group(a),d.type.get(g)=="array"&&g.forEach(function(a){console.info(a)}),j.forEach(function(a){console.warn(a)}),console.groupEnd());return h},addInitializer:function(a){d.initializers.push(a)},debug:function(a){d.debugMode&&
console.log("["+d.now()+"]",a)},now:function(){return(new Date).getTime()},__:function(a){return a.length==1?a[0]:a},upgradeObjectPrototype:function(){if(!Object.keys)Object.keys=function(a){var c=[],d;for(d in a)Object.prototype.hasOwnProperty.call(a,d)&&c.push(d);return c};if(!Object.forEach)Object.forEach=function(a,c,d){if(a===void 0||a===null)throw new TypeError;a=Object(a);if(typeof c!=="function")throw new TypeError;for(var h in a)a.hasOwnProperty(h)&&c.call(d,a[h],h,a)}},Selector:function(a,
c){return this.initialize(a,c)},Module:function(){return this},URL:function(a){return this.init(a)}};d.Selector.prototype={elements:[],initialize:function(a,e){if(typeof a==="string")if(c.Sizzle)this.elements=Sizzle(a,e);else try{e||(e=document);var d=e.querySelectorAll(a),h=d.length;this.elements=[];for(var j=0;j<h;j++)this.elements.push(d.item(j))}catch(k){console.log("Exception:",a)}else this.elements=Array.isArray(a)?a:[a];return this},identify:function(){this.forEach(function(a){if(!a.id)a.id=
"lola-guid-"+d.guid++});return this},get:function(a){a==void 0&&(a=0);return this.elements[a]},getAll:function(){return this.elements},count:function(){return this.elements.length},concat:function(a){if(a instanceof Selector)this.elements=this.elements.concat(a.getAll());else if(a instanceof Array)for(var c;c=a.pop();)this.concat(c);else this.elements.push(a);return this},safeDelete:function(){this.forEach(function(a){safeDelete(a)});return this}};d.Module.prototype={initialize:function(){},getNamespace:function(){return""},
getDependencies:function(){return[]},getSelectorMethods:function(){return{}}};d.URL.prototype={protocol:"",domain:"",path:"",page:"",vars:{},hash:"",init:function(a){this.parse(a);return this},parse:function(a){a=a.split("#",2);this.hash=a[1]?a[1]:"";var c={},a=a[0].replace(/[?&]+([^=&]+)=([^&]*)/gi,function(a,f,d){c[f]=d;return""});this.vars=c;a=a.split(":");a.length==2?(this.protocol=a[0],a=a[1].substr(2).split("/"),this.domain=a.shift()):a=a[0].split("/");this.page=a.pop();this.path=a.length>0?
(this.domain==""?"":"/")+a.join("/")+"/":""},toString:function(){var a=[];Object.forEach(this.vars,function(c,e){a.push(e+"="+c)});var c=a.length?"?"+a.join("&"):"",d=this.hash==""?"":"#"+this.hash;return this.protocol!=""?this.protocol+"://"+this.domain+this.path+this.page+c+d:this.path+this.page+c+d}};var a=function(a,c){return new d.Selector(a,c)};d.upgradeObjectPrototype();delete d.upgradeObjectPrototype;d.extend(a,d,true);d=a;c.$=d;c.lola=d;d.url=new d.URL(c.location.href);d.debugMode=d.url.vars.debug==
"true";document.readyState==="complete"?d.initialize(c):document.addEventListener?(document.addEventListener("DOMContentLoaded",d.initialize,false),c.addEventListener("load",d.initialize,false)):document.attachEvent&&(document.attachEvent("onreadystatechange",d.initialize),c.attachEvent("onload",d.initialize))})(window);
...
1;d.Grid.VERTICAL=2;d.Grid.prototype={x:0,y:0,width:100,height:100,spacing:10,flags:3,init:function(a,c,d,g,h,j){this.x=a||0;this.y=c||0;this.width=d||100;this.height=g||100;this.spacing=h||10;this.flags=j==void 0?3:j;return this},draw:function(a,f){var f=f==void 0?this.flags:f,d;if(f&c.chart.Grid.VERTICAL)for(d=this.x+this.spacing;d<=this.width+this.x;d+=this.spacing)a.beginPath(),a.moveTo(d,this.y),a.lineTo(d,this.y+this.height),a.stroke(),a.closePath();if(f&c.chart.Grid.HORIZONTAL)for(d=this.y+
...
"string"&&console.warn('command "'+d+'" has already been registered... overwriting');this.registry[d]=a;c.event.addListener(this,d,this.executeCommand)},execute:function(a,d,e){if(this.registry[a])if(e||(e=new g.Responder),typeof this.registry[a]=="string")this.callLater[a]||(c.loadScript(this.registry[a],function(){if(typeof this.registry[a]=="function"){for(var d=c.cmd.callLater[a].length,f=0;f<d;f++){var e=c.cmd.callLater[a][f];c.cmd.execute(e.name,e.params,e.responder)}delete c.cmd.callLater[a]}else throw Error('The command loaded from "'+
c.cmd.registry[a]+'" is not named "'+a+'"');}),this.callLater[a]=[]),this.callLater[a].push({name:a,params:d,responder:e});else{if(g=this.registry[a]){var g=new g;e&&(c.event.addListener(g,"result",e.handleResult),c.event.addListener(g,"fault",e.handleFault),c.event.addListener(g,"status",e.handleStatus));g.execute(d)}}else throw Error("Unknown command type: "+a);return e},executeCommand:function(a){c.cmd.execute(a.type,a.data.parameters,a.data.responder)},Responder:function(){return this.init()},
...
0,0,0.58,1);this.registerSimpleEasing("ease-in-out",0.42,0,0.58,1);delete c.easing.initialize},getNamespace:function(){return"easing"},getDependencies:function(){return["math.point","geometry"]},cubicBezier:function(a,d,e,g,h){var j=1-a;return c.math.point.add(c.math.point.multiply(d,j*j*j),c.math.point.multiply(e,3*j*j*a),c.math.point.multiply(g,3*j*a*a),c.math.point.multiply(h,a*a*a))},sampleSpline:function(a,c){var d=a.getPoints(),g=d.length-1,h=[];if(g>0){c*=g;for(var j=[],k=1;k<=g;k++)j.push(d[k].getAnchor().x);
for(var k=g=0,l=j[0],m=0;m<=c;m++){var n=m/c;n<=l?(n=(n-g)/(l-g),n=this.cubicBezier(n,d[k].getAnchor(),d[k].getControl2(),d[k+1].getControl1(),d[k+1].getAnchor()),h.push(n)):(k++,g=l,l=j[k],m--)}}return h},register:function(a,f,e,g){var g=g===true,e=f.getPoint(0).getAnchor(),h=f.getPoint(f.getPoints().length-1).getAnchor();if(e.x==0&&e.y==0&&h.x==1&&h.y==1)if(e=function(){return this},f=d.sampleSpline(f,1E3),e.prototype={samples:f,sampleCount:f.length,lastIndex:1,exec:function(a,c,d,f){a/=f;for(var e=
this.samples,f=this.lastIndex,g=this.sampleCount;a>e[f].x&&f<g;)if(f++,a<=e[f].x)return g=e[f-1],e=e[f],a=(a-g.x)/(e.x-g.x),this.lastIndex=f,c+d*(g.y+a*(e.y-g.y))}},!d.methods[a]||g)c.easing.methods[a]=e;else throw Error("easing id already taken");else throw Error("invalid easing spline");},registerSimpleEasing:function(a,f,e,g,h){var j=c.geometry,k=new j.Spline,f=new j.Point(f,e),g=new j.Point(g,h),h=f.toVector(),g=g.toVector();k.addPoint(new j.SplinePoint(0,0,0,0,h.velocity,h.angle));k.addPoint(new j.SplinePoint(1,
...
(function(c){var d={rDropPx:/px/g,preinitialize:function(){c.debug("lola.geometry::preinitialize");if(!c)throw Error("lola not defined!");delete c.geometry.preinitialize},initialize:function(){c.debug("lola.math.geom::initialize");if(!c)throw Error("lola not defined!");delete c.geometry.initialize},getNamespace:function(){return"geometry"},getDependencies:function(){return["math","regex"]},normalizeSpline:function(a,d,e,g,h,j,k){for(var j=j===true,k=k===true,l=c.math.point,m=new c.geometry.Spline,
a=a.getPoints(),n=a.length,o=l.subtract(e,d),q=l.subtract(h,g),e=function(a){a=l.divide(l.subtract(a,d),o);if(j)a.x=1-a.x;if(k)a.y=1-a.y;return l.multiply(a,q)},g=0;g<n;g++){var r=a[g].getControl1(),p=a[g].getAnchor(),h=a[g].getControl2(),p=e(p),r=l.subtract(p,e(r)).toVector(),h=l.subtract(e(h),p).toVector(),h=new c.geometry.SplinePoint(p.x,p.y,r.velocity,r.angle,h.velocity,h.angle);m.addPoint(h)}return m},getOffset:function(a,f){f||(f=false);var e=new d.Point(a.offsetLeft,a.offsetTop);if(f&&a.offsetParent)var g=
d.getOffset(a.offsetParent,true),e=c.math.point.add(e,g);return e},absolutePosition:function(a){return d.getOffset(a,true)},relativePosition:function(a){return d.getOffset(a,false)},width:function(a,d){if(d){var e=c.css.style(a,"borderLeft"),g=c.css.style(a,"borderRight"),h=c.css.style(a,"paddingLeft"),j=c.css.style(a,"paddingRight");d-=e+g+h+j;return c.css.style(a,"width",d)}else return a.offsetWidth?a.offsetWidth:a.clientWidth},height:function(a,d){if(d){var e=c.css.style(a,"borderTop"),g=c.css.style(a,
"borderBottom"),h=c.css.style(a,"paddingTop"),j=c.css.style(a,"paddingBottom");d-=e+g+h+j;return c.css.style(a,"height",d)}else return a.offsetHeight?a.offsetHeight:a.clientHeight},pointDistance:function(a,d){var e=c.math.point.subtract(d,a);return Math.sqrt(Math.pow(e.x,2)+Math.pow(e.y,2))},Point:function(a,c){this.x=a;this.y=c;return this},Spline:function(a,c){this.points=a?a:[];this.flags=c==void 0?0:c;return this},SplinePoint:function(a,c,d,g,h,j){return this.init(a,c,d,g,h,j)},Vector:function(a,
c){this.velocity=a;this.angle=c;return this},getSelectorMethods:function(){return{}}};d.Point.prototype={x:void 0,y:void 0,toVector:function(){var a=Math.atan2(this.y,this.x),d=Math.sqrt(this.x*this.x+this.y*this.y);return new c.geometry.Vector(d,a)},toString:function(){return"{x:"+this.x+",y:"+this.y+"}"}};d.Spline.CLOSED=1;d.Spline.FILL=2;d.Spline.STROKE=4;d.Spline.CONTROLS=8;d.Spline.prototype={points:[],flags:0,addPoint:function(a,c){if(c==void 0)c=this.points.length;this.points.splice(c,0,a)},
removePoint:function(a){a!=void 0&&this.points.splice(a,1,void 0)},updatePoint:function(a,c){c!=void 0&&this.points.splice(c,1,a)},getPoint:function(a){return this.points[a]},getPoints:function(){return this.points},draw:function(a,c){c=c==void 0?this.flags:c;if(this.points.length>1){var e=[];this.points.forEach(function(a){e.push(a.getControl1());e.push(a.getAnchor());e.push(a.getControl2())});var g=e.length;if(c&d.Spline.CONTROLS){a.beginPath();a.moveTo(e[1].x,e[1].y);a.lineTo(e[2].x,e[2].y);a.stroke();
a.closePath();for(var h=3;h<g-3;h+=3){var j=h+1,k=h+2;a.beginPath();a.moveTo(e[h].x,e[h].y);a.lineTo(e[j].x,e[j].y);a.stroke();a.closePath();a.beginPath();a.moveTo(e[j].x,e[j].y);a.lineTo(e[k].x,e[k].y);a.stroke();a.closePath()}a.beginPath();a.moveTo(e[h].x,e[h].y);a.lineTo(e[h+1].x,e[h+1].y);a.stroke();a.closePath()}a.beginPath();a.moveTo(e[1].x,e[1].y);for(h=2;h<g-3;h+=3)a.bezierCurveTo(e[h].x,e[h].y,e[h+1].x,e[h+1].y,e[h+2].x,e[h+2].y);c&d.Spline.CLOSED&&a.bezierCurveTo(e[g-1].x,e[g-1].y,e[0].x,
e[0].y,e[1].x,e[1].y);c&d.Spline.FILL&&a.fill();c&d.Spline.STROKE&&a.stroke();a.closePath()}else throw Error("not enough spline points");}};d.SplinePoint.prototype={anchor:void 0,entry:void 0,exit:void 0,init:function(a,d,e,g,h,j){this.anchor=new c.geometry.Point(a,d);this.entry=new c.geometry.Vector(e,g);this.exit=new c.geometry.Vector(h,j==void 0?g:j)},setAngle:function(a,c){this.entry.angle=a;this.exit.angle=c==void 0?a:c},getAnchor:function(){return this.anchor},getControl1:function(){return c.math.point.subtract(this.anchor,
this.entry.toPoint())},getControl2:function(){return c.math.point.add(this.anchor,this.exit.toPoint())}};d.Vector.prototype={velocity:void 0,angle:void 0,toPoint:function(){return new c.geometry.Point(Math.cos(this.angle)*this.velocity,Math.sin(this.angle)*this.velocity)}};c.registerModule(d)})(lola);
...
password:null,request:false,ready:false,init:function(a,c,d,g,h,j){this.method=c||"POST";this.headers=d||[];this.async=g===true;this.url=a;this.user=h;this.password=j;return this},getRequestObject:function(){var a=false;if(window.XMLHttpRequest&&!window.ActiveXObject)try{a=new XMLHttpRequest}catch(c){a=false}else if(window.ActiveXObject)try{a=new ActiveXObject("Msxml2.XMLHTTP")}catch(d){try{a=new ActiveXObject("Microsoft.XMLHTTP")}catch(g){a=false}}return a},makeRequest:function(a,d,e,g,h,j,k,l,m){var n=
this.getRequestObject();n.open(e,a,h,l,m);n.setRequestHeader("Content-type","application/x-www-form-urlencoded");for(a=0;a<g.length;a++)try{n.setRequestHeader(g[a].name,g[a].value)}catch(o){}if(d!=null&&c.type.get(d)!="string"){var g=[],q;for(q in d)g.push(q+"="+c.string.encode(d[q]));d=g.join("&")}n.onreadystatechange=function(){j.call(k)};n.send(d);return n},load:function(a){this.request=this.makeRequest(this.url,a,this.method,this.headers,true,this.readyStateChange,this,this.user,this.password)},
...
this.url),c.event.trigger(this,"fault",false,false,this.request))}},responseText:function(){return this.ready?this.request.responseText:false},responseXML:function(){return this.ready?this.request.responseXML:false}};d.AsyncRequest.prototype=d.Request.prototype;d.SyncRequest.prototype=d.Request.prototype;d.Transform.prototype={xml:null,xsl:null,xslParams:null,xmlParams:null,xslCacheId:"",resultNodes:[],init:function(a,d,e,g,h,j){this.xmlParams=d;this.xslParams=g;this.transformParams=h;this.xslCacheId=
j||"";if(c.type.get(e)=="string"){if(a=e,e=c.http.getCachedXsl(a),!e)throw Error('unknown xsl cache id: "'+a+'"');}else this.xsl=e;if(this.xsl&&this.xml)c.event.addListener(this.xsl,"result",this.checkStates,true,0,this),c.event.addListener(this.xsl,"fault",this.handleXSLFault,true,0,this),c.event.addListener(this.xml,"result",this.checkStates,true,0,this),c.event.addListener(this.xml,"fault",this.handleXMLFault,true,0,this),this.checkStates();else throw Error("transform error!");},checkStates:function(){if(this.xml.ready&&
...
(function(c){var d={cx:/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,escapable:/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,gap:null,indent:null,meta:{"\u0008":"\\b","\t":"\\t","\n":"\\n","\u000c":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},rep:null,handleResponse:{},ruid:0,preinitialize:function(){c.debug("lola.json::preinitialize");if(!c)throw Error("lola not defined!");
delete c.json.preinitialize},initialize:function(){c.debug("lola.json::initialize");if(!c)throw Error("lola not defined!");delete c.json.initialize},getNamespace:function(){return"json"},getDependencies:function(){return["http"]},escapeQuotes:function(a){return this.escapable.test(a)?'"'+a.replace(this.escapable,function(a){var d=c.json.meta[a];return typeof d==="string"?d:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+a+'"'},str:function(a,c){var d,g,h,j,k=this.gap,l,m=c[a];m&&typeof m===
"object"&&typeof m.toJSON==="function"&&(m=m.toJSON(a));typeof this.rep==="function"&&(m=this.rep.call(c,a,m));switch(typeof m){case "string":return this.escapeQuotes(m);case "number":return isFinite(m)?String(m):"null";case "boolean":case "null":return String(m);case "object":if(!m)return"null";this.gap+=this.indent;l=[];if(Object.prototype.toString.apply(m)==="[object Array]"){j=m.length;for(d=0;d<j;d+=1)l[d]=this.str(d,m)||"null";h=l.length===0?"[]":this.gap?"[\n"+this.gap+l.join(",\n"+this.gap)+
"\n"+k+"]":"["+l.join(",")+"]";this.gap=k;return h}if(this.rep&&typeof this.rep==="object"){j=this.rep.length;for(d=0;d<j;d+=1)g=this.rep[d],typeof g==="string"&&(h=this.str(g,m))&&l.push(this.escapeQuotes(g)+(this.gap?": ":":")+h)}else for(g in m)Object.hasOwnProperty.call(m,g)&&(h=this.str(g,m))&&l.push(this.escapeQuotes(g)+(this.gap?": ":":")+h);h=l.length===0?"{}":this.gap?"{\n"+this.gap+l.join(",\n"+this.gap)+"\n"+k+"}":"{"+l.join(",")+"}";this.gap=k;return h}},encode:function(a){return c.json.stringify(a)},
decode:function(a){return c.json.parse(a)},stringify:function(a,c,d){var g;this.indent=this.gap="";if(typeof d==="number")for(g=0;g<d;g+=1)this.indent+=" ";else if(typeof d==="string")this.indent=d;if((this.rep=c)&&typeof c!=="function"&&(typeof c!=="object"||typeof c.length!=="number"))throw Error("JSON.stringify");return this.str("",{"":a})},parse:function(a,c){function d(a,g){var k,l,m=a[g];if(m&&typeof m==="object")for(k in m)Object.hasOwnProperty.call(m,k)&&(l=d(m,k),l!==void 0?m[k]=l:delete m[k]);
return c.call(a,g,m)}var g,a=String(a);this.cx.test(a)&&(a=a.replace(this.cx,function(a){return"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)}));if(/^[\],:{}\s]*$/.test(a.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,"")))return g=eval("("+a+")"),typeof c==="function"?d({"":g},""):g;throw new SyntaxError("JSON.parse");},get:function(a,d,e){console.log("json.get: "+a);var g=new c.URL(a);
if(g.protocol=="____")console.log("    same domain"),a=new c.http.AsyncRequest(a),d&&c(a).addListener("result",function(a){console.log("    result");a=c.json.parse(a.data.responseText);d(a)}),a.load();else{console.log("    cross domain");var h=this.ruid++;c.json.handleResponse[h]=function(a){d(a);delete c.json.handleResponse[h]};g.vars[e?e:"jsonp"]="lola.json.handleResponse["+h+"]";c.loadScript(g.toString())}},getSelectorMethods:function(){return{}},upgradePrototypes:function(){if(typeof Date.prototype.toJSON!==
...
(function(c){var d={hooks:{},preinitialize:function(){c.debug("lola.template::preinitialize");if(!c)throw Error("lola not defined!");delete c.template.preinitialize},initialize:function(){c.debug("lola.template::initialize");if(!c)throw Error("lola not defined!");var a=c.now();c('script[type="text/x-lola-template"]').forEach(function(a){d.add(a.id,a.innerHTML)});var f=c.now();c.debug("templates parsed in "+(f-a)+" ms");delete c.template.initialize},getNamespace:function(){return"template"},getDependencies:function(){return["json"]},
add:function(a,c){if(!a||a=="getValue")throw Error("invalid template id");this.hooks[a]=new d.TemplateHook(c)},addHook:function(a,c){if(!a||a=="getValue")throw Error("invalid hook id");this.hooks[a]=new d.Hook(c)},getHook:function(a){if(!this.hooks[a])throw Error('hook "'+a+'" not found.');return this.hooks[a]},apply:function(a,d){var e="",g=c.template.getHook(a);g&&(e=g.evaluate(d));return e},Tag:function(a){return this.init(a)},Hook:function(a){return this.init(a)},TemplateHook:function(a){return this.init(a)},
getSelectorMethods:function(){return{applyTemplate:function(a,d){this.html(c.template.apply(a,d))}}}};d.Tag.prototype={rGetParts:/^([A-Za-z_$][A-Za-z0-9_$]*)(\[[^\]]+\])?(->[A-Za-z_$][A-Za-z0-9_$]*)?/,property:"",options:{},hookName:"",init:function(a){a&&this.parse(a);return this},parse:function(a){if(a=a.match(this.rGetParts))this.property=a[1],this.parseOptions(a[2]),this.hookName=a[3]?a[3].replace(/-\>/g,""):""},parseOptions:function(a){if(a){var a=a.slice(1,-1).trim(),c=0,d={};a.split(",").forEach(function(a){a=
a.split(":");a.length>1?d[a[0].trim()]=a[1].trim():d[String(c)]=a[0].trim();c++});this.options=d}else this.options={}},toString:function(){Object.keys(this.options);var a=[];this.options.forEach(function(c,d){a.push(d+":"+c)});return this.property+"["+a.join(",")+"]"+(this.hookName==""?"":"->"+this.hookName)},evaluate:function(a){a=a[this.property];if(Object.keys(this.options).length>0)switch(c.type.get(a)){case "boolean":a=this.options[a?"0":"1"];break;default:a=this.options[a]}this.hookName!=""&&
(a=c.template.getHook(this.hookName).evaluate(a));return a}};d.Hook.prototype={fn:null,init:function(a){if(typeof a==="function")this.fn=a;else throw Error("invalid hook.");return this},evaluate:function(a){return this.fn.apply(c.window,arguments)}};d.TemplateHook.prototype={rTag:/\$\{([^\}]+)\}/,blocks:[],blockCount:0,init:function(a){a&&this.parse(a);return this},parse:function(a){for(var c=[],e=a.search(this.rTag);e>=0;){var g=a.match(this.rTag),h=a.substring(0,e);h&&c.push(h);c.push(new d.Tag(g[1]));
a=a.substring(e+g[0].length);e=a.search(this.rTag)}a&&c.push(a);this.blocks=c;this.blockCount=c.length},evaluate:function(a){var d=[],e=this.blockCount,g=this.blocks;c.type.get(a)!="array"&&(a=[a]);a.forEach(function(a,c){for(var k=0;k<e;){var l=g[k];typeof l==="string"?d.push(String(l)):d.push(l.evaluate(a,c));k++}});return d.join("")}};c.registerModule(d)})(lola);
(function(c){var d={src:"tests.xml",tests:{},results:[],preinitialize:function(){c.debug("lola.test::preinitialize");if(!c)throw Error("lola not defined!");delete c.test.preinitialize},initialize:function(){c.debug("lola.test::initialize");if(!c)throw Error("lola not defined!");delete c.test.initialize},getNamespace:function(){return"test"},getDependencies:function(){return[]},setSource:function(a){d.src=a},run:function(){console.log("lola.test.run: "+d.src);var a=new c.http.SyncRequest(d.src);a.load();
a=a.responseXML();console.log(a)},Group:function(a){this.name=a;return this},Test:function(a){this.name=a;return this},getSelectorMethods:function(){return{}}};d.Group.prototype={tests:[],name:"",setup:function(){},execute:function(){this.tests.forEach(function(a){a.execute()})},teardown:function(){}};d.Test.prototype={name:"",test:"(function(){ return true; })();",expected:true,actual:void 0,passed:false,errorMsg:"",execute:function(){try{this.actual=eval(this.test),this.actual===this.expected?this.passed=
true:(this.passed=false,this.errorMsg="Expected: "+String(this.expected)+" Actual: "+String(this.actual))}catch(a){this.errorMsg="Error: "+a.message}}};c.registerModule(d)})(lola);
...
"tweenpause",false,false))},resume:function(a){this.tweens[a]&&(this.tweens[a].resume(),c.event.trigger(this.tweens[a],"tweenresume",false,false))},addTarget:function(a,d,e,g){if(this.tweens[a]){g=g===true;c.type.get(d)!="array"&&(d=[d]);for(var h=d.length,j=0;j<h;j++){var k=d[j],l=c(k).identify().attr("id");this.targets[l]||(this.targets[l]={});for(var m in e)if(m=="style")for(var n in e[m]){if(g||this.targets[l]["style:"+n]==null){if(!e[m][n].from&&!k.style[n]){var o=c.css.style(k,n);typeof e[m][n]==
"object"?e[m][n].from=o:e[m][n]={from:o,to:String(e[m][n])}}this.targets[l]["style:"+n]||(this.targets[l]["style:"+n]=[]);g?this.targets[l]["style:"+n].push(this.getTweenObject(a,k.style,n,e[m][n])):this.targets[l]["style:"+n]=[this.getTweenObject(a,k.style,n,e[m][n])]}}else this.targets[l][m]||(this.targets[l][m]=[]),g?this.targets[l][m].push(this.getTweenObject(a,k,m,e[m])):this.targets[l][m]=[this.getTweenObject(a,k,m,e[m])]}}else throw Error("tween not found");},getTweenObject:function(a,f,e,
g){var h,j,k;h=g.from?g.from:typeof g=="function"?g.call(f):f[e];var l=0;if(h!=null&&h!=void 0)if(c.type.isPrimitive(g))j=g;else if(g.to)l=0,j=g.to;else if(g.add)l=1,j=g.add;else{if(g.by)l=1,j=g.by}else throw Error("invalid tween parameters");var m,n;if(c.tween.hooks[e])m=c.tween.hooks[e];else for(var o in c.tween.types)if(m=c.tween.types[o],m.match.test(String(j))&&m.match.test(String(h)))break;else m=null;if(m)j=m.parse(j),h=m.parse(h),k=m.getDelta(j,h,l),n=m.proxy,m.canTween(h,j)||(m=null);if(!m)n=
c.tween.setAfterProxy,k=j;return new d.TweenObject(a,f,e,h,k,n)},tick:function(){var a=false,d=c.now(),e=c.tween.tweens,g;for(g in e)e[g].active&&(a=true,e[g].complete?(c.event.trigger(e[g],"tweencomplete",false,false),delete e[g],c.tween.freeTweenIds.push(parseInt(g))):e[g].calculate(d));var d=c.tween.targets,h;for(h in d){g=0;for(var j in d[h]){for(var k=[],l;l=d[h][j].shift();)l&&e[l.tweenId]&&e[l.tweenId].active&&(l.apply(e[l.tweenId].value),k.push(l));d[h][j]=k;d[h][j].length==0?delete d[h][j]:
